...
// PDF 專用的工作經歷數據 (與履歷頁面相同)
const experienceDataForPdf: Omit<TimelineItem, 'date'>[] = [
  {
...
// PDF 專用的教育背景數據
const educationDataForPdf: Omit<TimelineItem, 'date'>[] = [
  {
...
export default function CVDocument({ language }: { language: string }) {
  ...
  return (
    <div className="cv-document-wrapper">
      {/* 內聯樣式定義了 PDF 的整體外觀 */}
      <style>{`
        /* ... 此處省略大量 CSS 樣式定義，其功能與 class 名稱對應 ... */
        .cv-document-wrapper { background-color: #ffffff; color: #333333; font-family: 'Inter', 'Noto Sans TC', sans-serif; padding: 18px; width: 780px; box-sizing: border-box; line-height: 1.45; }
        ...
        .cv-profile-name { font-size: 34px; color: #212529; margin-bottom: 3px; font-weight: bold; }
        .cv-profile-title { font-size: 16px; color: ${accentColor}; font-weight: 500; }
        .cv-header-contact-details { text-align: right; font-size: 12.5px; color: #555555; line-height: 1.4; padding-top: 20px; }
        ...
        .cv-section-title-container { display: flex; align-items: center; margin-bottom: 11px; color: #2c3e50; }
        ...
        .cv-timeline-item { position: relative; margin-bottom: 12px; padding: 0px 12px 6px; background-color: #f8f9fa; border-radius: 4px; border: 1px solid #dee2e6; }
        ...
        .cv-timeline-title { font-size: ${language === 'en' ? '14.5px' : '16px'}; color: #2c3e50; font-weight: bold; margin-bottom: 2px; position: relative; top: -4px; }
        .cv-timeline-institution { font-size: ${language === 'en' ? '13px' : '14px'}; color: #6c757d; margin-bottom: 5px; position: relative; top: -4px; }
        .cv-timeline-description { font-size: 13px; color: #495057; line-height: 1.4; position: relative; top: -4px; }
        .cv-timeline-description strong { font-weight: bold; color: #2c3e50; }
        ...
        .cv-skill-name { color: #212529; font-weight: 500; }
        .cv-skill-level-text { color: ${accentColor}; }
        .cv-skill-bar-bg { width: 100%; background-color: #e9ecef; border-radius: 4px; height: 7px; position: relative; top: 6px; }
        ...
        .cv-tool-item { font-size: 13px; color: #212529; font-weight: 600; padding: 3px 0; position: relative; padding-left: 22px; text-align: left; }
